...
REM Build static lib
    ...
    lib /OUT:TLETC_WIN32.lib %EXT_OBJS% 
)
IF "%1%" == "" (
echo NORMAL BUILD
REM Normal build
cl %COMPILE_FLAGS% %INCLUDE_PATH% %WARNS_DISABLE% %DEFINES% %TARGETS% /link /SUBSYSTEM:WINDOWS /OUT:TLETC_WIN32.exe %LIBRARY_PATH% %LIBRARIES%
)
